...
, list_t *apiKeys_token
    ...
    apiClient->progress_data = NULL;
    apiClient->response_code = 0;
    if(apiKeys_user!= NULL) {
        apiClient->apiKeys_user = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_user) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_user, pairDup);
        }
    }else{
        apiClient->apiKeys_user = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    apiClient->progress_func = NULL;
    apiClient->progress_data = NULL;
    if(apiClient->apiKeys_user) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_user) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_user);
    }
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_user != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_user) {
